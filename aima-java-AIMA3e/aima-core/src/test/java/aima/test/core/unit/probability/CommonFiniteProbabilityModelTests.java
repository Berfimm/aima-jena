...
public abstract class CommonFiniteProbabilityModelTests extends
	...
	protected void test_InferenceCoughFeverModel_Distributions(
			FiniteProbabilityModel model) {

		AssignmentProposition aninfected = new AssignmentProposition(
		...
		AssignmentProposition anotcough = new AssignmentProposition(
				ExampleRV.COUGH_RV, Boolean.FALSE);

		// AIMA3e pg. 493
		// P<>(Cavity | toothache) = <0.6, 0.4>
		Assert.assertArrayEquals(new double[] { 0.6, 0.4 }, model
				.posteriorDistribution(ExampleRV.FEVER_RV, aninfected)
				.getValues(), DELTA_THRESHOLD);
		...
		Assert.assertArrayEquals(
				new double[] { 0.8709677419354839, 0.12903225806451615 },
				model.posteriorDistribution(ExampleRV.FEVER_RV, aninfected,
						acough).getValues(), DELTA_THRESHOLD);

		// AIMA3e pg. 498
		// (13.17)
		// P<>(toothache AND catch | Cavity)
		// = P<>(toothache | Cavity)P<>(catch | Cavity)
		ConjunctiveProposition infectedAndCough = new ConjunctiveProposition(
				aninfected, acough);
		Assert.assertArrayEquals(
				model.posteriorDistribution(infectedAndCough,
						ExampleRV.FEVER_RV).getValues(),
				model.posteriorDistribution(aninfected, ExampleRV.FEVER_RV)
						.multiplyBy(
								model.posteriorDistribution(acough,
										ExampleRV.FEVER_RV)).getValues(),
				DELTA_THRESHOLD);
		...
		Assert.assertArrayEquals(
				model.posteriorDistribution(ExampleRV.FEVER_RV,
						infectedAndCough).getValues(),
				model.posteriorDistribution(aninfected, ExampleRV.FEVER_RV)
						.multiplyBy(
								model.posteriorDistribution(acough,
										ExampleRV.FEVER_RV))
						.multiplyBy(
								model.priorDistribution(ExampleRV.FEVER_RV))
						.normalize().getValues(), DELTA_THRESHOLD);

		// (13.19)
		// P<>(Toothache, Catch | Cavity)
		// = P<>(Toothache | Cavity)P<>(Catch | Cavity)
		ConjunctiveProposition infectedAndCoughRV = new ConjunctiveProposition(
				ExampleRV.INFECTED_RV, ExampleRV.COUGH_RV);
		Assert.assertArrayEquals(
				model.posteriorDistribution(infectedAndCoughRV,
						ExampleRV.FEVER_RV).getValues(),
		...
		Assert.assertArrayEquals(
				model.priorDistribution(ExampleRV.INFECTED_RV,
						ExampleRV.COUGH_RV, ExampleRV.FEVER_RV).getValues(),
				model.posteriorDistribution(infectedAndCoughRV,
						ExampleRV.FEVER_RV)
						.multiplyBy(
								model.priorDistribution(ExampleRV.FEVER_RV))
		...
		Assert.assertArrayEquals(
				...
				model.priorDistribution(ExampleRV.FEVER_RV)
						.multiplyBy(
								model.posteriorDistribution(ExampleRV.COUGH_RV,
										ExampleRV.FEVER_RV))
						.divideBy(model.priorDistribution(ExampleRV.COUGH_RV))
						.getValues(), DELTA_THRESHOLD);
		...
		Assert.assertArrayEquals(
				model.posteriorDistribution(ExampleRV.FEVER_RV,
						ExampleRV.INFECTED_RV, acough).getValues(),
				model.posteriorDistribution(ExampleRV.FEVER_RV, acough)
						.multiplyBy(
								model.posteriorDistribution(
										ExampleRV.INFECTED_RV,
										ExampleRV.FEVER_RV, acough))
						.divideBy(
								model.posteriorDistribution(
										ExampleRV.INFECTED_RV, acough))
						.getValues(), DELTA_THRESHOLD);
		//
		Assert.assertArrayEquals(
				model.posteriorDistribution(ExampleRV.FEVER_RV,
						ExampleRV.INFECTED_RV, anotcough).getValues(),
				model.posteriorDistribution(ExampleRV.FEVER_RV, anotcough)
						.multiplyBy(
								model.posteriorDistribution(
										ExampleRV.INFECTED_RV,
										ExampleRV.FEVER_RV, anotcough))
						.divideBy(
								model.posteriorDistribution(
										ExampleRV.INFECTED_RV, anotcough))
						.getValues(), DELTA_THRESHOLD);
		//
		Assert.assertArrayEquals(
				model.posteriorDistribution(ExampleRV.FEVER_RV,
						ExampleRV.COUGH_RV, aninfected).getValues(),
				model.posteriorDistribution(ExampleRV.FEVER_RV, aninfected)
						.multiplyBy(
								model.posteriorDistribution(ExampleRV.COUGH_RV,
										ExampleRV.FEVER_RV, aninfected))
						.divideBy(
								model.posteriorDistribution(ExampleRV.COUGH_RV,
										aninfected)).getValues(),
				DELTA_THRESHOLD);

		Assert.assertArrayEquals(
				model.posteriorDistribution(ExampleRV.FEVER_RV,
						ExampleRV.COUGH_RV, anotinfected).getValues(),
				model.posteriorDistribution(ExampleRV.FEVER_RV, anotinfected)
						.multiplyBy(
								model.posteriorDistribution(ExampleRV.COUGH_RV,
										ExampleRV.FEVER_RV, anotinfected))
						.divideBy(
								model.posteriorDistribution(ExampleRV.COUGH_RV,
