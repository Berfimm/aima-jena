...
public class ModelBasedReflexVacuumAgent extends SimpleAgent<VacuumPercept, Action> {
	...
	private static Set<Rule<Action>> getRuleSet() {
		...
		//I added Wet location state,action Wipe and I changed Suck to Sweep
		rules.add(new Rule<>(new EQUALCondition(ATT_CURRENT_STATE, LocationState.Dirty), ACTION_SWEEP));
