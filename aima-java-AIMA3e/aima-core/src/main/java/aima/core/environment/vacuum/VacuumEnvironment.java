 ...
 * and <em></em>. Assume for the moment, that ing is 100% effective. The
...
public class VacuumEnvironment extends AbstractEnvironment<VacuumPercept, Action> {
	...
	public static final Action ACTION_SWEEP = new DynamicAction("Sweep"); //Defined action for dirty
	public static final Action ACTION_WIPE = new DynamicAction("Wipe"); //Defined action for wet
	...
	public enum LocationState {
		Clean, Dirty, Wet//new state
	}

	/**
	 * Constructs a vacuum environment with two locations A and B, in which dirt AND WET are
	 * placed at random.
	 */
	...
	public VacuumEnvironment() {
		...
		this(Util.randomBoolean() ? LocationState.Clean : LocationState.Dirty,
				Util.randomBoolean() ? LocationState.Dirty : LocationState.Wet);//new environment
	}
	...
	public void execute(Agent<?, ?> agent, Action action) {
		...
		//I changed  ACTION_SUCK to ACTION_SWEEP
			...
			//I added new Wipe action with performance measure.
